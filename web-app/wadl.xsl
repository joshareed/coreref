...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0"
    ...
    xmlns:ns="urn:namespace" extension-element-prefixes="exsl" xmlns="http://www.w3.org/1999/xhtml"
        ...
        <xsl:element name="resource" namespace="{$wadl-ns}">
            ...
            <xsl:choose>
                <xsl:when test="$uri">
                    ...
                    <xsl:attribute name="id"><xsl:value-of select="@type"/>#<xsl:value-of
...
-->
            ...
            <head>
                <title>
                    <xsl:choose>
                        ...
                        <xsl:otherwise>Services Resource Descriptions</xsl:otherwise>
                    </xsl:choose>
                </title>
                <style type="text/css">
                    body {
            ...
            <body>
                <h1>
                    <xsl:choose>
                        ...
                        <xsl:otherwise>Service Resource Description</xsl:otherwise>
                    </xsl:choose>
                </h1>
                ...
                <ul>
                    ...
                    <xsl:if test="descendant::wadl:fault">
                        <li>
                            ...
                            <ul>
                                <xsl:apply-templates
                                    select="exsl:node-set($resources)/descendant::wadl:fault"
                                    mode="toc"/>
                            </ul>
                        </li>
                    </xsl:if>
                ...
                <xsl:apply-templates
                    select="exsl:node-set($resources)/descendant::wadl:representation" mode="list"/>
                                ...
                                <xsl:for-each
                                    ...
                                    <xsl:choose>
                                        <xsl:when test="@required='true'">
                                            <xsl:choose>
                                                <xsl:when
                                                  test="preceding-sibling::wadl:param[@style='query']"
                                                  >&amp;</xsl:when>
    ...
    <xsl:template match="wadl:*" mode="param-group">
        ...
        <xsl:if test="ancestor-or-self::wadl:*/wadl:param[@style=$style]">
            <h6><xsl:value-of select="$prefix"/><xsl:text> </xsl:text><xsl:value-of select="$style"
                /> parameters</h6>
            <table>
                <tr>
                    <th>parameter</th>
        ...
        <tr>
            ...
            <td>
                ...
                <xsl:choose>
                    ...
                    <xsl:otherwise>
                        <xsl:if test="@default">
                            <p>Default: <tt><xsl:value-of select="@default"/></tt></p>
                        </xsl:if>
                        <xsl:if test="@fixed">
                            <p>Fixed: <tt><xsl:value-of select="@fixed"/></tt></p>
                        </xsl:if>
                    </xsl:otherwise>
                </xsl:choose>
            </td>
    ...
    <xsl:template match="wadl:link">
        <li> Link: <a href="#{@resource_type}"><xsl:value-of select="@rel"/></a>
        </li>
    </xsl:template>
    ...
    <xsl:template name="get-element">
        ...
        <p>
            <em>Source: <a href="{$source}"><xsl:value-of select="$source"/></a></em>
        </p>
